...
def cli(interactive: bool, files_path: str, frame_type: str = 'WIB', map_id: str = "HDColdbox", threshold: int = 20, out_file: str = ".") -> None:
    ...
    rtpc_df = pd.read_hdf(files_path, 'test')
    rich.print(rtpc_df)

    tpgm = TPGManager(500, "data/fir_coeffs.dat", 6, threshold)
    tp_df, ped_df, fir_df = tpgm.run_capture(rtpc_df, 0, 0, pedchan=True, align=False)

    rich.print(tp_df)
    tp_df.to_hdf(out_file, "emulator_rtp")
    rich.print(ped_df)
